classdef FFT_3D_file_data < handle  
    properties
        fileAddress
        nameOfDataSet
        pointNumber
        coordinate
        scanType
        outputType
        componentType
        measurementType
        frequencies
        realData
        imaginaryData
        dataHeaders
        units
        rawData
    end
    properties (Hidden)
        fileID
        
    end
    methods
        function obj = FFT_3D_file_data(fileToBeRead)
            if nargin ==1
                obj.fileAddress = fileToBeRead;
            end            
        end 
        function parse(obj)
            obj.fileID = fopen(obj.fileAddress);
            obj.parse_header();
            obj.parse_data();
            fclose(obj.fileID);
        end
        function parse_header(obj)
            for count = 1:7
                obj.parse_header_line(fgetl(obj.fileID));
            end                
        end
        function parse_data(obj)
            scanString = repmat('%f', 1, length(obj.units));
            obj.rawData = textscan(obj.fileID, scanString);
            obj.parse_raw_data;
        end  
        function parse_header_line(obj, fileLine)
            splitStrings = strsplit(fileLine, ':');
            if(length(splitStrings) == 2)
                obj.parse_file_info(splitStrings)
            else
                splitStrings = strsplit(fileLine, '\t');
                if ~isempty(splitStrings{1})
                    obj.parse_data_units(splitStrings)
                end
            end
        end
        function parse_file_info(obj, splitStrings)
            switch splitStrings{1}
                case 'Source File Name'
                    obj.parse_file_name(splitStrings{2});
                case 'Point Index'
                    obj.parse_point_data(splitStrings{2});
                case 'Component'
                    obj.parse_component(splitStrings{2});
                case 'Signal'
                    obj.parse_signal(splitStrings{2});
                otherwise
                    warning('not expected data format')
            end
        end
        function parse_data_units(obj, splitStrings)
            if contains(splitStrings{1}, '[')
                obj.units = erase(erase(splitStrings, '['), ']');
            else
                obj.dataHeaders = splitStrings;
                for i = 2:length(obj.dataHeaders)
                    obj.dataHeaders{i} = strcat(splitStrings{i},' -', obj.measurementType(4:end));
                end
            end            
        end        
        function parse_file_name(obj, string)
            obj.nameOfDataSet = strtrim(string);
        end
        function parse_point_data(obj, string)
            string(strfind(string, '=')) = [];
            obj.pointNumber = sscanf(string, '%f');
            x = parse_for_x(string);
            y = parse_for_y(string);
            z = parse_for_z(string);
            obj.coordinate = [x,y,z];
        end
        function parse_component(obj, string)
            string = strsplit(string, '(');
            obj.componentType = strtrim(string{1});
        end
        function parse_signal(obj, string)
            signal = strsplit(string, ' - ');
            obj.scanType = strtrim(signal{1});
            obj.measurementType = strtrim(signal{2});
            obj.outputType = strtrim(signal{3});
        end
        function parse_raw_data(obj)
            obj.parse_frequencies();
            obj.parse_real_data();
            obj.parse_imaginary_data(); 
        end
        function parse_frequencies(obj)
            obj.frequencies = obj.rawData{strcmp(obj.dataHeaders(:), 'Frequency')};
        end
        function parse_real_data(obj)
            col = find(contains(obj.dataHeaders(:), 'Real'));
            if length(col) > 1
                obj.realData = obj.parse_component_parts(col);
            else
            obj.realData = obj.rawData{col};
            end
        end
        function parse_imaginary_data(obj)
            col = find(contains(obj.dataHeaders(:), 'Imaginary'));
            if length(col) > 1
                obj.imaginaryData = obj.parse_component_parts(col);
            else
            obj.imaginaryData = obj.rawData{col};
            end
        end
        function data = parse_component_parts(obj, col)
            data = zeros(length(obj.frequencies), length(col));
            for c = 1:length(col)
                coordinate = obj.find_XYZ(obj.dataHeaders(col(c)));
                data(:, coordinate) = obj.rawData{col(c)};
            end
        end
        
        function choose_file_to_parse(obj)
            obj.fileAddress = uigetfile(pwd, 'Select a file to parse');
        end
    end
    methods (Static)
        function xyz = find_XYZ(header)
            if contains(header, 'X')
                xyz = 1;
            elseif contains(header, 'Y')
                xyz = 2;
            elseif contains(header, 'Z')
                xyz = 3;
            else
                warning('No dimension information')
                xyz = NaN;
            end
        end
    end
end
function number = parse_for_number_after_key(pointDataString, Key)
    Index = strfind(pointDataString, Key);
    number = sscanf(pointDataString(Index(1) + length(Key):end), '%g', 1);
end
function value = parse_for_x(pointDataString)
    value = parse_for_number_after_key(pointDataString, 'x');
end
function value = parse_for_y(pointDataString)
    value = parse_for_number_after_key(pointDataString, 'y');
end
function value = parse_for_z(pointDataString)
    value = parse_for_number_after_key(pointDataString, 'z');
end
