classdef quinticSplineBasisEvaluator < handle
    ...
    methods (Hidden = true)
        ...
        function [values, d1, d2, d3] = matrix_closed_loop_basis_derivatives(obj, parameter)
            ...
            for param_index = 1:length(parameter)
                ...
                % pad localBu to full length
                padNumber = obj.numberOfControlPoints-length(localBu);
                Bu = padarray(localBu,[0 padNumber], 0,'post');
                deriv1 = padarray(ld1, [0 padNumber], 0,'post');
                deriv2 = padarray(ld2, [0 padNumber], 0,'post');
                deriv3 = padarray(ld3, [0 padNumber], 0,'post');
                % shift to correct location
                shift = obj.order- leftknotIndex ;
                % put basis values into correct part of full basis values vector
                values(param_index, :) = circshift(Bu, shift);
